...
export class Uri {
   ...
   constructor(uri?: string, options?: IUriParseOptions) {
    ...
    private _setFromUriString(uriString: string, options?: IUriParseOptions) {
        ...
        let hashSplit = this._singleSplit(uri, "#");
        if (hashSplit) {
            uri = hashSplit.part1;
        ...
        let querySplit = this._singleSplit(uri, "?");
        if (querySplit) {
    ...
    */
    public getQueryParam(name: string): string | null | undefined {
        ...
        if (this.queryParameters) {
            let matchingPairs = this.queryParameters.filter(p => stringEquals(p.name, name, true));
